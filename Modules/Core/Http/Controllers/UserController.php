    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $request->validate([
                ...
                'role_assignments.*.role_id' => 'nullable|exists:roles,id',
                'role_assignments.*.school_id' => 'nullable|exists:schools,id',
                'school_id' => 'nullable|exists:schools,id',
                'phone' => 'nullable|string|max:50',
                'address' => 'nullable|string|max:255',
        ...
        $data['is_active'] = $request->has('is_active');
        $data['force_password_reset'] = $request->has('force_password_reset');
        $data['primary_role_id'] = $request->input('primary_role_id') ?: null;
        if ($request->hasFile('avatar')) {
            $data['avatar'] = $request->file('avatar')->store('avatars', 'public');
        }
        $user->update($data);
        // Assign roles per school
        if ($request->has('role_assignments') && !empty($request->role_assignments)) {
            $syncData = [];
            foreach ($request->role_assignments as $assignment) {
