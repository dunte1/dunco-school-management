...
<div class="container">
    <h1>Allocate Room</h1>
    <form action="{{ route('hostel.room_allocations.store') }}" method="POST">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="allocation_type" id="manual" value="manual" {{ old('allocation_type', 'manual') == 'manual' ? 'checked' : '' }} onclick="toggleAllocationType()">
                <label class="form-check-label" for="manual">Manual</label>
            </div>
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="allocation_type" id="auto" value="auto" {{ old('allocation_type') == 'auto' ? 'checked' : '' }} onclick="toggleAllocationType()">
                <label class="form-check-label" for="auto">Auto</label>
            </div>
            @error('allocation_type')<div class="text-danger">{{ $message }}</div>@enderror
        </div>
        <div id="manualFields" style="display: {{ old('allocation_type', 'manual') == 'manual' ? 'block' : 'none' }};">
            <div class="mb-3">
        ...
        <div id="autoFields" style="display: {{ old('allocation_type') == 'auto' ? 'block' : 'none' }};">
            <div class="mb-3">
                ...
                <select name="preferences[gender]" id="preferences_gender" class="form-control">
                    ...
                    <option value="male" {{ old('preferences.gender') == 'male' ? 'selected' : '' }}>Male</option>
                    <option value="female" {{ old('preferences.gender') == 'female' ? 'selected' : '' }}>Female</option>
                    <option value="mixed" {{ old('preferences.gender') == 'mixed' ? 'selected' : '' }}>Mixed</option>
                </select>
            </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="check_in" id="check_in" class="form-control" value="{{ old('check_in') }}">
