...
<div class="container mx-auto p-4">
    ...
    <form action="{{ route('finance.fees.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="amount" step="0.01" value="{{ old('amount') }}" class="w-full border rounded px-3 py-2 @error('amount') border-red-500 @enderror" required>
            @error('amount')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="fee_category_id" class="w-full border rounded px-3 py-2 @error('fee_category_id') border-red-500 @enderror">
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" @if(old('fee_category_id') == $category->id) selected @endif>{{ $category->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="fee_type_id" class="w-full border rounded px-3 py-2 @error('fee_type_id') border-red-500 @enderror">
                ...
                @foreach($types as $type)
                    <option value="{{ $type->id }}" @if(old('fee_type_id') == $type->id) selected @endif>{{ $type->name }}</option>
                @endforeach
            </select>
            @error('fee_type_id')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
