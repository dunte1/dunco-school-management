...
class NewFeeAssigned extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject('New Fee Assigned for ' . $this->fee->student->name)
            ->greeting('Dear Parent/Guardian,')
            ->line('A new fee has been assigned for your child:')
            ->line('Student: ' . $this->fee->student->name)
            ->line('Category: ' . ($this->fee->category->name ?? 'N/A'))
