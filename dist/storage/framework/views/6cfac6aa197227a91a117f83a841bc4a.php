        ...
        <div>
            <div class="form-floating">
                <select name="status" id="status" class="form-select">
                    ...
                    <option value="rejected" <?php if(request('status')=='rejected'): ?> selected <?php endif; ?>>Rejected</option>
                </select>
        ...
        <div class="table-responsive">
            <table class="table table-modern align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <?php if($excuse->student): ?>
                                <span class="text-info"><i class="bi bi-person"></i> <?php echo e($excuse->student->full_name ?? $excuse->student->name); ?></span>
                            <?php elseif($excuse->staff): ?>
                                <span class="text-warning"><i class="bi bi-person-badge"></i> <?php echo e($excuse->staff->first_name); ?> <?php echo e($excuse->staff->last_name); ?></span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo e($excuse->date->format('Y-m-d')); ?></td>
                        <td><?php echo e($excuse->period); ?></td>
                        <td><?php echo e($excuse->reason); ?></td>
                        <td>
                            <?php if($excuse->document): ?>
                                <a href="<?php echo e(asset('storage/'.$excuse->document)); ?>" target="_blank" class="btn btn-outline-info btn-sm btn-modern">
                        ...
                        <td>
                            <span class="badge rounded-pill bg-<?php echo e($excuse->status == 'pending' ? 'warning text-dark' : ($excuse->status == 'approved' ? 'success' : 'danger')); ?>">
                                <i class="bi <?php echo e($excuse->status == 'pending' ? 'bi-hourglass-split' : ($excuse->status == 'approved' ? 'bi-check-circle' : 'bi-x-circle')); ?>"></i>
                                <?php echo e(ucfirst($excuse->status)); ?>

                            </span>
                        </td>
                        <td>
                            <?php if($excuse->status == 'pending'): ?>
                            <form method="POST" action="<?php echo e(route('academic.attendance.excuse.action', $excuse->id)); ?>" class="d-inline">
                                <?php echo csrf_field(); ?>
                                <input type="hidden" name="action" value="approve">
                                <button type="submit" class="btn btn-success btn-sm btn-modern"><i class="bi bi-check"></i> Approve</button>
                            </form>
                            <form method="POST" action="<?php echo e(route('academic.attendance.excuse.action', $excuse->id)); ?>" class="d-inline ms-1">
                                <?php echo csrf_field(); ?>
...
<?php $__env->stopPush(); ?> 
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\dunth\dunco school management system\duncoschool\dist\Modules\Academic\resources\views\attendance\excuses.blade.php ENDPATH**/ ?>