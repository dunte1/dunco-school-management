            ...
            <div class="col-lg-7 col-md-9">
                <div class="card shadow rounded-4 border-0">
                    <div class="card-header bg-white d-flex justify-content-between align-items-center border-0 pb-0">
                        <h3 class="fw-bold mb-0"><i class="fas fa-star-half-alt text-warning me-2"></i> Edit Performance Review</h3>
                    ...
                    <div class="card-body p-4">
                        ...
                        @endif
                        <form action="{{ route('hr.performance_reviews.update', $review->id) }}" method="POST" autocomplete="off">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                ...
                                <select name="staff_id" id="staff_id" class="form-select" required>
                                    ...
                                    @forelse($staff as $s)
                                        <option value="{{ $s->id }}" {{ $review->staff_id == $s->id ? 'selected' : '' }}>{{ $s->first_name }} {{ $s->last_name }}</option>
                                    @empty
                            ...
                            <div class="mb-3">
                                ...
                                <select name="reviewer_id" id="reviewer_id" class="form-select" required>
                                    ...
                                    @forelse($staff as $s)
                                        <option value="{{ $s->id }}" {{ $review->reviewer_id == $s->id ? 'selected' : '' }}>{{ $s->first_name }} {{ $s->last_name }}</option>
                                    @empty
                                        <option value="">No staff available</option>
                                    @endforelse
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="period" id="period" class="form-control" required value="{{ $review->period }}" placeholder="e.g., 2025-Q3 or 2025-Annual">
                            </div>
                            <div class="mb-3">
                                <label for="score" class="form-label">Score (1-10) <span class="text-danger">*</span></label>
                                <input type="number" name="score" id="score" class="form-control" min="1" max="10" value="{{ $review->score }}" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="comments" id="comments" class="form-control" rows="3">{{ $review->comments }}</textarea>
                            </div>
