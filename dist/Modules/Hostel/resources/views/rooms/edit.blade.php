...
<div class="container">
    ...
    <form action="{{ route('hostel.rooms.update', $room) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="checkbox" name="ac" id="ac" value="1" {{ old('ac', $room->ac) ? 'checked' : '' }}>
            @error('ac')<div class="text-danger">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="ensuite" class="form-label">Ensuite</label>
            <input type="checkbox" name="ensuite" id="ensuite" value="1" {{ old('ensuite', $room->ensuite) ? 'checked' : '' }}>
            @error('ensuite')<div class="text-danger">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="capacity" class="form-label">Capacity</label>
            <input type="number" name="capacity" id="capacity" class="form-control" value="{{ old('capacity', $room->capacity) }}" required min="1">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="amenities[]" id="amenities" class="form-control" value="{{ old('amenities', is_array($room->amenities) ? implode(',', $room->amenities) : $room->amenities) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="layout_image" id="layout_image" class="form-control" value="{{ old('layout_image', $room->layout_image) }}">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" name="price_per_bed" id="price_per_bed" class="form-control" value="{{ old('price_per_bed', $room->price_per_bed) }}">
