...
class ChatBotService
{
    ...
    public function __construct(OpenAIService $openAIService, ConversationService $conversationService)
    {
    ...
    protected function generateAIResponse(string $userMessage, Conversation $conversation, array $context = [])
    {
        ...
        // Add context
        $contextData = $this->buildContext($context);
        if (!empty($contextData)) {
            $messages[] = ['role' => 'system', 'content' => $contextData];
        }

        try {
            $response = $this->openAIService->generateResponse($userMessage, $context);
            return $response['message'] ?? 'I apologize, but I encountered an error processing your request.';
        } catch (\Exception $e) {
            Log::error('OpenAI Response Error: ' . $e->getMessage());
