...
const applyFilters = () => {
    router.get(route('nursing.attendance.index'), {
...
<template>
    <Head title="Clinical Attendance" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold text-gray-800">Clinical Attendance</h2>
        </template>
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="entry in attendance.data" :key="entry.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span :class="[statusColors[entry.status] || 'bg-gray-100 text-gray-800', 'px-2 py-1 text-xs font-medium rounded-full inline-flex items-center gap-1']">
                                        <span>{{ statusIcons[entry.status] || '' }}</span>
                                        {{ entry.status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <Link :href="route('nursing.attendance.show', entry.id)" class="text-brand-blue hover:underline">View</Link>
                                </td>
                            </tr>
                ...
                <!-- Pagination -->
                <div v-if="attendance.last_page > 1" class="mt-4 flex items-center justify-between">
                    <p class="text-sm text-gray-500">
                        Page {{ attendance.current_page }} of {{ attendance.last_page }} ({{ attendance.total }} total)
                    </p>
                    <div class="flex gap-2">
                        <button
                            @click="goToPage(attendance.current_page - 1)"
                            :disabled="attendance.current_page <= 1"
                        ...
                        <button
                            @click="goToPage(attendance.current_page + 1)"
                            :disabled="attendance.current_page >= attendance.last_page"
                            class="px-3 py-1.5 text-sm bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            Next
                        </button>
